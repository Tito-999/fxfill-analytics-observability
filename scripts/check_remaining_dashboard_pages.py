...
def check_sql_contracts(db_path: str) -> dict:
    ...
    try:
        rows = conn.execute(
            "SELECT MIN(event_date), MAX(event_date) FROM main_marts.mart_feature_adoption_segmented"
        ).fetchone()
        d1, d2 = rows[0], rows[1]
        if d1 and d2:
            rows2 = conn.execute(
            ...
            for r in rows2:
                ...
                if adopted < 0 or adopted > total:
                    violations.append(f"adopted({adopted})>total({total})")
                if rate < 0 or rate > 1:
                    violations.append(f"rate={rate}")
            results["feature_adoption"] = {
                "passed": ok and len(violations) == 0,
                "row_count": len(rows2),
                "failures": violations,
            }
        else:
    ...
    try:
        d1, d2 = conn.execute(
            "SELECT MIN(first_use_date), MAX(first_use_date) FROM main_marts.mart_feature_time_to_first_use"
        ).fetchone()
        if d1 and d2:
            rows2 = conn.execute(
    ...
    try:
        rows2 = conn.execute(
            "SELECT user_id, experiment_group, total_tasks, successful_tasks, task_success_rate, avg_field_accuracy, avg_agent_latency_ms, total_cost_usd FROM main_marts.mart_ab_test_user_metrics ORDER BY experiment_group, user_id"
        ).fetchall()
        ok = len(rows2) > 0
        violations = []
        for r in rows2:
            if r[2] and r[3] and r[3] > r[2]:
                violations.append(f"exported>{r[2]}")
            if r[4] and (r[4] < 0 or r[4] > 1):
                violations.append(f"rate={r[4]}")
            if r[6] and r[6] < 0:
                violations.append(f"latency={r[6]}")
            if r[7] and r[7] < 0:
                violations.append(f"cost={r[7]}")
        results["ab_test"] = {
            "passed": ok and len(violations) == 0,
            "row_count": len(rows2),
            "failures": violations,
...
def check_page_rendering(db_path: str) -> dict:
    ...
    results = {
        ...
        "passed": 0,
        "failures": [],
        "catalog_exception_count": 0,
        "binder_exception_count": 0,
        "streamlit_exception_count": 0,
    }
    ...
    for name, path in pages.items():
        try:
            ...
            for et in exc_types:
                if "Catalog" in et:
                    results["catalog_exception_count"] += 1
                if "Binder" in et:
                    results["binder_exception_count"] += 1
                if "Streamlit" in et:
                    results["streamlit_exception_count"] += 1
                ...
                results["failures"].append(
                    {
                        ...
                        "errors": (
                            exc_types[:3]
                            if exc_types
                            else [str(e.value)[:100] for e in app.exception[:3]]
                        ),
                    }
                )
        except Exception as e:
...
def main():
    ...
    if report["accepted"]:
        print(
            f"ACCEPTED: {page_results['passed']}/3 pages, {sum(1 for v in sql_results.values() if v['passed'])}/4 SQL contracts"
        )
        sys.exit(0)
    else:
        print(
            f"FAILED: pages={page_results['passed']}/{page_results['checked']}, SQL={sum(1 for v in sql_results.values() if v['passed'])}/{len(sql_results)}"
        )
        sys.exit(1)
