...
[![发布基线测试](https://img.shields.io/badge/v1.2.12%20Baseline%20Tests-406%20%2F%20406%20passed-brightgreen.svg)](https://github.com/Tito-999/fxfill-analytics-observability)
...
- 核心验收：`true`
- 必检发布门禁：11 / 11 `PASS`
- pytest（`portfolio-v1.2.12` 发布基线）：406 / 406 通过
- dbt 模型：41 / 41 成功执行
- dbt 测试：44 / 44 通过
...
| **漏斗与留存** | 转化漏斗、相邻步骤转化率、精确日留存队列、考虑成熟度的周留存以及用户生命周期分析 |
| **功能采用** | 功能级使用趋势和采用率 |
...
| `portfolio-v1.2.12` pytest 发布基线 | 406 / 406 通过 |
| 必检发布门禁 | 11 / 11 PASS |
...
- **`portfolio-v1.2.12` pytest 发布基线：** 406 / 406 通过，0 失败、错误或跳过。
- **dbt 模型：** 41 / 41 成功执行。
...
**版本说明：** `0.1.0` 是 `pyproject.toml` 中声明的 Python 软件包/API 版本；`portfolio-v1.2.12` 是不可变的作品集与可复现制品发布标签。两者用途不同，因此不要求数值一致。
