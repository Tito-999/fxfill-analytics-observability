...
**Warehouse:** A 4-layer DuckDB warehouse with 7 raw tables, 7 staging views, 13 intermediate views, and 21 analytics marts, all built through dbt-core with 44 dbt tests (21 generic, 23 singular). Cross-layer phenomena reconciliation confirms Python-derived metrics match dbt SQL values within tolerance.
...
**Quality Assurance:** An automated pytest verification suite with machine-verifiable JSON audit evidence for every phase. The immutable `portfolio-v1.2.12` release records a 406/406 pytest baseline. 20 documented SQL interview queries are included. All data is synthetic. No real users, revenue, or production data is included.
